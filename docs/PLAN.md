...
架构基线见 [ARCHITECTURE.md](ARCHITECTURE.md)。沿既有局域网 Android 客户端范围推进，不修改服务端部署。

路由：RESUME / IN_PROGRESS；认证和外部 API 适配属于 HIGH 风险，新应用沿用 FULL 设计基线，当前处于 VERIFY 与缺陷修正阶段。任务清单以本表为唯一来源，质量结果以 VERIFICATION.md 为唯一来源。
...
| 真实环境 | Android 测试设备 | 安装、登录、连续切换、退后台、失败恢复 | PARTIAL：需要使用者在自己的设备和服务环境中复验 |

交付包含源码、架构、构建脚本、调试 APK 和验证记录。不会把网页端播放成功等同于 Android 端通过。

测试环境修复记录：曾对临时 Android 测试环境进行可回退的连接修复；应用架构不变。删除数据卷、替换未知镜像或扩大网络暴露不在处置范围。
